...
parser.add_argument('-c', '--gpu', default='2', type=str,
...
def compute_errors(ground_truth, pre):
    ...
    return PSNR, SSIM, l1, TV





def evaluation(real, fake, flag, filename, batch_size, cuda, dims):
    ...
    for file in real_files:
        path_real = os.path.join(real, file)
        real_img = Image.open(path_real).resize([256,256]).convert('RGB')
        path_fake = os.path.join(fake, file)
        
        if not os.path.isfile(path_fake):
            raise FileNotFoundError('')

        fake_img = Image.open(path_fake).resize([256,256]).convert('RGB')

        num += 1
        '''if num % 100 == 0:
            print(num)'''
            
        psnr, ssim, l1, tv= compute_errors(real_img, fake_img)
        mean_psnr += psnr
        mean_ssim += ssim
        mean_l1 += l1
        mean_tv += tv
    ...
    M_psnr = mean_psnr / len(real_files)
    M_ssim = mean_ssim / len(real_files)
    M_l1 = mean_l1 / len(real_files)
    M_tv = mean_tv / len(real_files)
    
    print('mean-PSNR %f dB' % M_psnr)
    print('mean-SSIM %f' % M_ssim)
    print('mean-L1 %f' % M_l1)
    print('mean-TV %f' % M_tv)
    ...
    if not os.path.exists(os.path.dirname(filename)):
        os.makedirs('{:s}'.format(os.path.dirname(filename)))
    ...
    txt_file.write('\n')
    txt_file.write('mean-PSNR: %f , mean-SSIM: %f , FID: %f , KID: %f, mean-L1: %f, mean-TV: %f' % (M_psnr, M_ssim, fid_value, mmd2s.mean() * 100, M_l1, M_tv))
...
if __name__ == '__main__':
    ...
    for i in range(8):
        ...
        print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
        print(str(base + i * interval))
        print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
        evaluation('./test_oxford/20_50/real' + str(base + i * interval),
                    './test_oxford/20_50/only' + str(base + i * interval),
                    'iteration_' + str(base + i * interval), 
                    './result_oxford/20_50/evaluation.txt',
                    args.batch_size,
                    args.gpu != '',
                    args.dims)